...
async def run_check(check_id: int, db: Session) -> None:
    """Main pipeline. Runs in background task."""
    ...
    try:
        # Step 1: Parse file (read + LLM extraction)
        t0 = time.monotonic()
        items = await parse_supplier_file(check.file_path)
        timings["1_parse_file"] = round(time.monotonic() - t0, 1)
        logger.info(f"[check={check_id}] Step 1 parse_file: {timings['1_parse_file']}s, found {len(items) if items else 0} items")
        ...
        _progress[check_id] = {"total": len(items), "processed": 0, "status": "processing", "message": "Проверка товаров..."}

        # Step 2: Check items in parallel
        ...
        _progress[check_id] = {
            "total": _progress.get(check_id, {}).get("total", 0),
            "processed": _progress.get(check_id, {}).get("processed", 0),
            "status": "error",
